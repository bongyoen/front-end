    ...
    stage('Server Run') {
      steps {
        sshagent(credentials: [SSH_CONNECTION_CREDENTIAL]) {
          ...
          sh "ssh -o StrictHostKeyChecking=no ${SSH_CONNECTION} 'docker run -d --name ${IMAGE_NAME} -v /reactUploads:/app/build/uploads -p 80:80 ${IMAGE_STORAGE}/${IMAGE_NAME}:latest'"
