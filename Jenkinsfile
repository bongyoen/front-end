    ...
    stage('Server Run') {
      steps {
        sshagent(credentials: [SSH_CONNECTION_CREDENTIAL]) {
          ...
          sh "ssh -o StrictHostKeyChecking=no ${SSH_CONNECTION} 'docker run -d --name ${IMAGE_NAME} -v /home/ec2-user/reactVolume:/app/build/uploads/localdir -p 80:80 ${IMAGE_STORAGE}/${IMAGE_NAME}:latest'"
